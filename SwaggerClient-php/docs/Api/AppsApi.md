...
```php
appsByNameNameGet($name, $org_id): \Wodby\Api\Model\App
```
...
try {
    $result = $apiInstance->appsByNameNameGet($name, $org_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AppsApi->appsByNameNameGet: ', $e->getMessage(), PHP_EOL;
}
```
