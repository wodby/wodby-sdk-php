...
```php
createAppBuild($create_build_request): \Wodby\Api\Model\AppBuildsCreateResponse
```
...
[**\Wodby\Api\Model\AppBuildsCreateResponse**](../Model/AppBuildsCreateResponse.md)
...
```php
getAppBuildConfig($id): \Wodby\Api\Model\AppBuildConfig
```
...
try {
    $result = $apiInstance->getAppBuildConfig($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AppBuildsApi->getAppBuildConfig: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\Wodby\Api\Model\AppBuildConfig**](../Model/AppBuildConfig.md)
